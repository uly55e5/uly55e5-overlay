...
inherit eutils elisp-common toolchain-funcs scons-utils 
...
EDDIR="${WORKDIR}/${MY_PN}-Source/editors"
...
src_install() {
	...
	if use vim; then
		cd ${EDDIR}/scvim
		escons CUSTOMCCFLAGS="${CFLAGS}"	CUSTOMCXXFLAGS="${CXXFLAGS}" \
			PREFIX="${D}/usr"  VIMFILE_DIR="${D}/usr/share/vim/vimfiles" \
			CACHE_DIR="${D}/usr/share/scvim/doc" \
			install-system install-vimfiles || die "scvim installation failed"
	fi

	if use gedit; then
		cd ${EDDIR}/sced
		escons CUSTOMCCFLAGS="${CFLAGS}"	CUSTOMCXXFLAGS="${CXXFLAGS}" \
			PREFIX="${D}/usr" install ||  die "scvim installation failed"
